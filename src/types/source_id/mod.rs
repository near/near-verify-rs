...
mod tests {
    ...
    fn test_source_id_from_url() {
        for (full_rev_url, remote_path_exp) in [
            (
            ...
            (
                "git+https://github.com/repo/sample_no_workspace?rev=10415b1359c74b0d5774ce08b114f2bd1a85445d",
                "/repo/sample_no_workspace",
            ),
        ] {
            println!(
                "case, full_rev_url, path_exp: {} {}",
                full_rev_url, remote_path_exp
            );
            let cargo_source_id = SourceId::from_url(full_rev_url).unwrap();
            ...
            assert_eq!(remote_path_exp, cargo_source_id.url().path());
        }
    }

    #[test]
    fn test_for_git() {
        for (remote_url, full_rev_url_exp) in [
            (
                "https://github.com/repo/sample_no_workspace.git",
                "git+https://github.com/repo/sample_no_workspace.git?rev=10415b1359c74b0d5774ce08b114f2bd1a85445d",
            ),
            (
                "https://github.com/repo/sample_no_workspace",
                "git+https://github.com/repo/sample_no_workspace?rev=10415b1359c74b0d5774ce08b114f2bd1a85445d",
            ),
        ] {
            println!(
                "case, remote_url, full_rev_url_exp: {} {}",
                remote_url, full_rev_url_exp
            );
            let url: url::Url = remote_url.parse().unwrap();
            ...
            assert_eq!(
                full_rev_url_exp.to_string(),
                format!("{}", source_id.as_url())
            )
        }
    }
}
